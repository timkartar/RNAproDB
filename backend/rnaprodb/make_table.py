...
def makeTable(json_output, path=None):
    ...
    table['Steric clashes'] = ["Node 1,Node 2,Distance,Overlap,Atom1,Atom2"]
    ...
    for item in data['links']:
        ...
        for key in tooltip:
            if "Hbond" in key and "Water" not in key:
                ...
                node2 = tooltip["Node2"]
                idx = key.split(" ")[1]
                distance = tooltip["distance_"+idx].replace("Angstrom", "Å")
                ...
                nt_distance = tooltip["nt_distance_"+idx].replace("Angstrom", "Å")
                aa_distance = tooltip["aa_distance_"+idx].replace("Angstrom", "Å")
